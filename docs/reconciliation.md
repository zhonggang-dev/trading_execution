...
仍有 OPEN、可重试的订单级问题（含 `ORDER_RECOVERY_STALLED`）时，该订单独立于普通回溯窗口
进入恢复集；即使已是 FILLED、预占已结清，也必须重新执行订单级成交核验，不能仅凭本地终态
关闭旧问题。恢复集按订单去重，继续遵守同一租约、超时和持久化退避；核验成功并关闭问题后
自动退出此恢复集。历史人工财务问题不因这个入口而被自动放行。
