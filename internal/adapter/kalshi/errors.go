...
// validation/FOK failures were not accepted; request timeout, conflict, and
// server failures remain ambiguous for state-changing calls.
func mapKalshiHTTPError(method string, status int, headers http.Header, body []byte) error {
	...
	if kalshiMethodMayMutate(method) && (status == http.StatusRequestTimeout || status == http.StatusConflict || status >= 500) {
		kind = port.VenueErrorAmbiguous
	} else if status >= 500 {
...
func classifyKalshiError(status int, apiCode, message string) string {
	...
	case strings.Contains(combined, "fill_or_kill") || strings.Contains(combined, "fill-or-kill") ||
		strings.Contains(combined, "fill or kill") || strings.Contains(combined, "fok") ||
		strings.Contains(combined, "could not be filled") || strings.Contains(combined, "cannot be filled") ||
		strings.Contains(combined, "not enough liquidity"):
