...
// buildRisks 返回由 Go 服务端计算的预警线、硬上限和执行状态。
func buildRisks(params buildRisksParams) ([]domain.LiveRisk, error) {
	...
	definitions := []riskDefinition{
		{id: "exposure", name: "总敞口", current: params.positionTotals.cost, hardLimit: params.exposureLimit, hardLimitEnforced: true, thresholdType: domain.LiveRiskHardLimit, unit: "$", hint: "全局所有钱包真实持仓成本，包含外部未纳管仓位"},
		{id: "market", name: "单市场最大敞口", current: params.positionTotals.maxMarketCost, hardLimit: maxMarketLimit, hardLimitEnforced: true, thresholdType: domain.LiveRiskHardLimit, unit: "$", hint: "当前敞口最大的账户内市场"},
		{id: "daily_volume", name: "当日交易金额", current: dailyCurrent, hardLimit: dailyLimit, hardLimitEnforced: true, thresholdType: domain.LiveRiskHardLimit, unit: "$", hint: "按各账户 daily_timezone 统计已验真成交加活动预占"},
		{id: "stale", name: "预测过期", current: big.NewRat(staleCount, 1), hardLimit: new(big.Rat), hardLimitEnforced: false, thresholdType: domain.LiveRiskTarget, unit: "count", hint: "超过各账户 signal_age 的持仓数量，运营目标为 0"},
	}
...
func makeRisk(definition riskDefinition) (domain.LiveRisk, error) {
	...
	hardLimit, err := numberFromRat(definition.hardLimit)
	if err != nil {
		return domain.LiveRisk{}, err
	}
	warningRat := riskWarningThreshold(definition.hardLimit)
	warning, err := numberFromRat(warningRat)
	if err != nil {
		return domain.LiveRisk{}, err
	}
	usage, err := riskUsagePercentage(definition.current, definition.hardLimit)
	if err != nil {
		return domain.LiveRisk{}, err
	}
	return domain.LiveRisk{
		ID: definition.id, Name: definition.name, Current: current,
		WarningThreshold: warning, HardLimit: hardLimit, Limit: hardLimit, UsagePercentage: usage,
		...
		State: riskState(definition.current, warningRat, definition.hardLimit),
	}, nil
}

// riskWarningThreshold 使用硬上限的 80% 作为统一服务端预警线。
func riskWarningThreshold(hardLimit *big.Rat) *big.Rat {
	return new(big.Rat).Mul(hardLimit, big.NewRat(4, 5))
}

// riskUsagePercentage 计算相对硬上限的真实百分比，目标为零时不返回无意义比例。
func riskUsagePercentage(current *big.Rat, hardLimit *big.Rat) (*domain.LiveNumber, error) {
	if hardLimit.Sign() == 0 {
		return nil, nil
	}
	percentage := new(big.Rat).Mul(new(big.Rat).Quo(current, hardLimit), big.NewRat(100, 1))
...
// riskState 区分安全、达到预警线和达到或超过硬上限三种状态。
func riskState(current *big.Rat, warning *big.Rat, hardLimit *big.Rat) domain.LiveFlowState {
	if hardLimit.Sign() == 0 {
		if current.Sign() > 0 {
