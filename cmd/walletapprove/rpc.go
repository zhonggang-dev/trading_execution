...
func readAllowance(ctx context.Context, rpc rpcCaller, owner, spender, blockTag string) (*big.Int, error) {
	data, err := buildAllowanceCallData(owner, spender)
	if err != nil {
		return nil, err
	}
	result, err := rpc.Call(ctx, "eth_call", []any{
		map[string]string{"to": polygonPUSDAddress, "data": "0x" + hex.EncodeToString(data)},
		blockTag,
	})
	if err != nil {
		return nil, fmt.Errorf("read pUSD allowance: %w", err)
	}
	decoded, err := parseRPCData(result, 32, "pUSD allowance")
	if err != nil {
		return nil, err
	}
...
func simulateApproval(ctx context.Context, rpc rpcCaller, owner, spender string) error {
	data, err := buildApproveCallData(spender, approvalAmount)
	if err != nil {
		return err
	}
	call := map[string]string{
		"from": owner, "to": polygonPUSDAddress, "value": "0x0", "data": "0x" + hex.EncodeToString(data),
	}
	result, err := rpc.Call(ctx, "eth_call", []any{call, "pending"})
	if err != nil {
		return fmt.Errorf("simulate pUSD approval: %w", err)
	}
	decoded, err := parseRPCData(result, 32, "approval simulation result")
	if err != nil {
		return err
	}
	for _, value := range decoded[:31] {
		if value != 0 {
			return fmt.Errorf("approval simulation returned non-canonical bool")
		}
	}
	if decoded[31] != 1 {
		return fmt.Errorf("approval simulation did not return true")
	}
	return nil
}

func estimateApprovalGas(ctx context.Context, rpc rpcCaller, owner, spender string) (uint64, error) {
	data, err := buildApproveCallData(spender, approvalAmount)
	if err != nil {
		return 0, err
	}
	...
	estimate, err := rpcQuantityUint64(ctx, rpc, "eth_estimateGas", []any{call, "pending"}, "approval gas estimate")
	if err != nil {
		return 0, err
	}
	if estimate < minimumApprovalGasEstimate || estimate > maximumApprovalGasLimit {
		return 0, fmt.Errorf("approval gas estimate %d is outside [%d,%d]", estimate, minimumApprovalGasEstimate, maximumApprovalGasLimit)
	}
	if estimate > (^uint64(0))/approvalGasNumerator {
		return 0, fmt.Errorf("approval gas margin overflows")
	}
	gasLimit := (estimate*approvalGasNumerator + approvalGasDenominator - 1) / approvalGasDenominator
	if gasLimit > maximumApprovalGasLimit {
		return 0, fmt.Errorf("approval gas limit %d exceeds cap %d", gasLimit, maximumApprovalGasLimit)
	}
	return gasLimit, nil
}
...
func requireNativeGasBalance(ctx context.Context, rpc rpcCaller, owner string, gasLimit uint64, maxFee *big.Int) error {
	maximumCost := new(big.Int).Mul(new(big.Int).SetUint64(gasLimit), maxFee)
	return requireNativeGasBudget(ctx, rpc, owner, maximumCost)
}

func requireNativeGasBudget(ctx context.Context, rpc rpcCaller, owner string, required *big.Int) error {
	if required == nil || required.Sign() <= 0 || required.BitLen() > 256 {
		return fmt.Errorf("account %s required native gas budget is invalid", owner)
	}
	balance, err := rpcQuantityBig(ctx, rpc, "eth_getBalance", []any{owner, "pending"}, "native balance")
	if err != nil {
		return err
	}
	if balance.Cmp(required) < 0 {
		return fmt.Errorf("account %s native balance %s is below required maximum gas budget %s", owner, balance, required)
	}
	return nil
}

func sendRawTransaction(ctx context.Context, rpc rpcCaller, signed signedApprovalTransaction) error {
...
func validateRPCTransaction(actual rpcTransaction, expected journalEntry) error {
	...
	if err != nil || value.Sign() != 0 {
		return fmt.Errorf("RPC transaction value is not zero")
	}
	data, err := normalizeHexData(actual.Input, "transaction input")
	if err != nil || data != expected.CallData {
		return fmt.Errorf("RPC transaction calldata mismatch")
	}
	return nil
}
...
func validateApprovalReceipt(receipt rpcReceipt, expected journalEntry) (validatedReceipt, error) {
	...
	if err != nil || status != 1 {
		return validatedReceipt{}, fmt.Errorf("approval transaction reverted or has invalid status")
	}
	...
	if err := validateApprovalLog(receipt.Logs, expected, blockNumber, blockHash); err != nil {
		return validatedReceipt{}, err
	}
	return validatedReceipt{blockNumber: blockNumber, blockHash: blockHash, gasUsed: gasUsed}, nil
}

func validateApprovalLog(logs []rpcLog, expected journalEntry, blockNumber uint64, blockHash string) error {
	eventTopic := "0x" + hex.EncodeToString(keccak256([]byte("Approval(address,address,uint256)")))
	ownerTopic, _ := addressTopic(expected.Owner)
	spenderTopic, _ := addressTopic(expected.Spender)
	matches := 0
	for index, logEntry := range logs {
		address, addressErr := normalizeAddress(logEntry.Address)
		if addressErr != nil || address != polygonPUSDAddress || len(logEntry.Topics) == 0 || strings.ToLower(logEntry.Topics[0]) != eventTopic {
			continue
		}
		if len(logEntry.Topics) != 3 || strings.ToLower(logEntry.Topics[1]) != ownerTopic || strings.ToLower(logEntry.Topics[2]) != spenderTopic {
			continue
		}
		data, err := normalizeFixedHex(logEntry.Data, 32, "approval log amount")
		if err != nil || new(big.Int).SetBytes(mustDecodeHex(data[2:])).Cmp(approvalAmount) != 0 {
			continue
		}
		logTransactionHash, err := normalizeFixedHex(logEntry.TransactionHash, 32, "approval log transaction hash")
		if err != nil || logTransactionHash != expected.TransactionHash {
			return fmt.Errorf("approval log %d transaction hash mismatch", index)
		}
		logBlockHash, err := normalizeFixedHex(logEntry.BlockHash, 32, "approval log block hash")
		if err != nil || logBlockHash != blockHash {
			return fmt.Errorf("approval log %d block hash mismatch", index)
		}
		logBlockNumber, err := parseQuantityUint64(logEntry.BlockNumber, "approval log block number")
		if err != nil || logBlockNumber != blockNumber {
			return fmt.Errorf("approval log %d block number mismatch", index)
		}
		if _, err := parseQuantityUint64(logEntry.LogIndex, "approval log index"); err != nil {
			return err
		}
		if logEntry.Removed == nil || *logEntry.Removed {
			return fmt.Errorf("approval log %d is missing removed=false", index)
		}
		matches++
	}
	if matches != 1 {
		return fmt.Errorf("receipt contains %d exact pUSD Approval logs; require 1", matches)
	}
	return nil
}
